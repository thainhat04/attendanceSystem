...
const contractAddress = "0xc12daed0728C2ef21e20503bA5270b2e0aa8ddD0";
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document.getElementById("markAttendance").addEventListener("click", () => {
    const classList = document.getElementById("classList"); // Lấy select
    if (!classList) {
        alert("❌ Không tìm thấy danh sách lớp!");
        return;
    }

    const classId = classList.value; // Lấy giá trị được chọn
    if (!classId) {
        alert("❌ Vui lòng chọn lớp!");
        return;
    }

    console.log("📌 classId đã chọn:", classId); // Kiểm tra xem classId có được lấy hay không
...
async function loadApprovedClasses(selectId) {
  if (!contract) {
    console.error("❌ Hợp đồng chưa được tải hoặc chưa khởi tạo.");
    return;
  }
  ...
  try {
    ...
    select.innerHTML = `<option value="" disabled selected>Chọn lớp</option>`;

    for (let i = 0; i < classIds.length; i++) {
      let option = document.createElement("option");
      option.value = classIds[i];
      option.textContent = `No.${classIds[i]}: ${classNames[i]}`;
      option.dataset.className = classNames[i];
...
async function loadPendingClasses() {
    ...
    if (!result || result.length < 3) {
      console.error("❌ Dữ liệu lớp chờ duyệt không hợp lệ:", result);
      return;
    }
    ...
    console.log("✅ Danh sách lớp chờ duyệt đã được cập nhật.");
  } catch (error) {
...
async function signAttendance(classId) {
  if (!window.ethereum) {
      alert("⚠️ Bạn cần cài MetaMask!");
      return;
  }
  ...
  try {
      ...
      const message = `Diem danh lop ${classId}`;
      const messageHash = web3.utils.keccak256(`\x19Ethereum Signed Message:\n${message.length}${message}`);
      const signature = await web3.eth.personal.sign(messageHash, accounts[0]);

      console.log("✅ Chữ ký số:", signature);
      return signature;
  } catch (error) {
      console.error("❌ Lỗi khi ký chữ ký số:", error);
      alert("❌ Lỗi khi tạo chữ ký!");
  }
}
async function markAttendance(classId) {
  if (!window.ethereum) {
    alert("⚠️ Cần cài MetaMask!");
    return;
  }
  ...
  try {
    ...
    if (!accounts || accounts.length === 0) {
      alert("⚠️ Không tìm thấy tài khoản MetaMask!");
      return;
    }

    const userAddress = accounts[0];
    console.log("🔹 Tài khoản MetaMask:", userAddress);
    ...
    console.log("📌 Đang ký với MetaMask...");
    const signature = await window.ethereum.request({
      method: 'personal_sign',
      params: [message, userAddress]
    });
    ...
    console.log("✅ Chữ ký hợp lệ:", signature);
    console.log("📌 Gửi giao dịch điểm danh lên blockchain...");

    // Lấy giá gas hiện tại
    const gasPrice = await web3.eth.getGasPrice();
    console.log("⛽ Giá gas hiện tại:", gasPrice);

    // Gửi giao dịch
    const tx = await contract.methods.markAttendance(
      parseInt(classId),
      true,
      signature
    ).send({ 
    ...
    console.log("✅ Điểm danh thành công:", tx);
    ...
    if (error.code === -32603) {
      alert("❌ Lỗi Internal JSON-RPC: Có thể do gas limit không đủ hoặc lỗi trong hợp đồng thông minh.");
    } else {
